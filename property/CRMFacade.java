...
public class CRMFacade {
    ...
    ArrayList<User> PropertiyUsers = new ArrayList<>();
public boolean addUser(User newuser)
    {
        this.PropertiyUsers.add(newuser);
        return true;
    }
    boolean removeUser(User toremove)
    {
        this.PropertiyUsers.remove(toremove);
        return true;
    }
    boolean authenticateUser(String user, String pass)
    {
        for(int i=0; i<PropertiyUsers.size(); i++)
        {
            if(this.PropertiyUsers.get(i).getUsername().equalsIgnoreCase(user) &&
                    this.PropertiyUsers.get(i).getPassword().equals(pass))
            {
                return true;
            }
    ...
    User getUserByUsername(String user)
    {
        for(int i=0; i<PropertiyUsers.size(); i++)
        {
            if(this.PropertiyUsers.get(i).getUsername().equals(user))
            {
                return this.PropertiyUsers.get(i);
            }
        }
        return null;
    }
